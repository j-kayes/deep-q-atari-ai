...
class Agent: 
    ...
    def build_model(self, fc_layers=4, layer_connections=128, hold_rate=1.0):
        ...
        self.output_layer = fully_connected(self.fc_layer[fc_layers-1], self.env.action_space.n) # Final layer connected to self.env.action_space.n possible actions.
    ...
    def get_samples(self, stop_after_limit=True, n_games=500000, max_t=250, epsilon=1.0, display_frames=False):
        # Will append the memory with state/action/reward data and return the average score across games:
        game_counter = 0
        scores = []
        frames = 0
    ...
    def train_network(self, target_mean_score=250.0, games=200000, batch_size=32, initial_epsilon=1.0, final_epsilon=0.05, epsilon_frames_range=30000, gamma=0.95, score_sample_size=250):
        ...
        for game in range(games):
            ...
            if(len(scores) == score_sample_size):
                scores = scores[1:] # Removing first element after a maximum sample size reached.
